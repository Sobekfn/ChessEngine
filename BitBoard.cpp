...
//sets a bit to 1
void BitBoard::set_PieceBit(PieceType piece, int square){
    boards[piece] |= (1ULL << square);
}

//checks if a piece of a given type exists on a square
bool BitBoard::get_PieceBit(PieceType piece, int square){
    return boards[piece] & (1ULL << square);
}


//removes a piece of a given type on a square
void BitBoard::pop_PieceBit(PieceType piece, int square){
        boards[piece] &= ~(1ULL << square);
}
